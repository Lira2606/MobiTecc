...
export function LoginForm() {
  ...
  const handleSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: isRecovering ? 'Falha na Redefinição' : (isRegistering ? 'Falha no Cadastro' : 'Falha no Login'),
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center w-full h-full relative">
      <div className={cn("flex-1 flex flex-col items-center justify-center p-8 w-full", isMounted ? 'login-screen-loaded' : 'opacity-0')}>
        ...
        <div className="login-element text-center w-full">
          <h2 className="text-3xl font-bold text-white">
            {isRecovering ? 'Redefinir Senha' : (isRegistering ? 'Criar Conta' : 'Bem-vindo!')}
          </h2>
        </div>
        <div className="login-element text-center w-full">
          <p className="text-gray-400 mt-2 mb-8">
            {isRecovering ? 'Digite seu email e a nova senha.' : (isRegistering ? 'Preencha os dados abaixo.' : 'Entre para continuar.')}
          </p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="w-full space-y-4 z-10">
            ...
            {isRegistering && !isRecovering && (
              <div className="login-element fade-in-up">
                <FormField
                    ...
                    <FormItem>
                      <FormControl>
                        <div className="relative">
                          ...
                          <Input {...field} placeholder="Nome Completo" className="w-full pl-11 pr-4 py-3 h-auto glass-effect rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-75" />
                        </div>
                      </FormControl>
                  ...
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input {...field} type={passwordVisible ? 'text' : 'password'} placeholder={isRecovering ? "Nova Senha" : "Senha"} className="w-full pl-11 pr-10 py-3 h-auto glass-effect rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-75" />
            ...
            <div className="pt-2 login-element">
              <Button type="submit" disabled={isSubmitting} className="w-full bg-teal-500 hover:bg-teal-600 text-white font-bold py-3 h-auto px-4 rounded-lg transition duration-300 transform active:scale-95">
                {isSubmitting ? <Loader2 className="animate-spin" /> : (isRecovering ? 'Redefinir Senha' : (isRegistering ? 'Cadastrar' : 'Entrar'))}
              </Button>
            </div>
          </form>
        </Form >
        ...
        <div className="login-element w-full z-10"><p className="text-center text-gray-400 text-sm mt-8">
          {isRecovering ? 'Lembrou a senha? ' : (isRegistering ? 'Já tem uma conta? ' : 'Não tem uma conta? ')}
          <button onClick={isRecovering ? toggleRecovery : toggleMode} className="text-teal-400 hover:text-teal-300 font-medium transition focus:outline-none">
            {isRecovering ? 'Entrar' : (isRegistering ? 'Entrar' : 'Cadastre-se')}
          </button>
        </p></div>
      </div >
    </div >
